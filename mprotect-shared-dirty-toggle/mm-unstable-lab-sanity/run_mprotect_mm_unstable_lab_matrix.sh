...
STAMP="${STAMP:-$(date -u +%Y%m%dT%H%M%SZ)_lab_mprotect_mm_unstable}"
CONFIGS="${CONFIGS:-1:14336 2:14336 4:14336 8:16384 16:32768}"
PROFILE="${PROFILE:-mm_regression_gen/mprotect/experiments/mprotect_shared_dirty_mm_unstable_check.toml}"
VERSION_FILE="${VERSION_FILE:-mm_regression_gen/out/kernel_versions_mprotect_mm_unstable.json}"
SEED="${SEED:-20260518}"
JOBS="${JOBS:-8}"
WORKLOAD_ROUNDS="${WORKLOAD_ROUNDS:-5}"
EXTRA_CMDLINE_VALUE="${MM_REGRESSION_EXTRA_CMDLINE:-tsc=unstable clocksource=refined-jiffies}"
EXTRA_CONFIG_ENABLE="${MM_REGRESSION_EXTRA_CONFIG_ENABLE:-SMP,ACPI,ACPI_PROCESSOR}"
...
limit_mib="$(memory_limit_mib)"
{
    ...
    echo "purpose=lab clean timing check for mprotect shared-dirty workload against v6.12/v6.19/mm-unstable"
...
if [[ ! -d linux-mm-unstable ]]; then
    {
        ...
        echo "Skipping mprotect mm-unstable lab matrix rather than silently falling back."
...
for cfg in ${CONFIGS}; do
    ...
    echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] start mprotect cpu=${cpu} mem=${mem} out=${out}" | tee -a "${LOG_ROOT}/progress.log"
    ...
    python3 mm_regression_gen/run_regression_pipeline.py "${PROFILE}" \
      ...
      --out-dir "${out}" \
      > "${log_file}" 2>&1
    echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] done mprotect cpu=${cpu} mem=${mem}" | tee -a "${LOG_ROOT}/progress.log"
done
